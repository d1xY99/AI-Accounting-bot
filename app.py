...
_layout = "wide" if st.session_state.page in ("kif", "kuf", "dnevni", "herbavital") else "centered"
...
# ═══════════════════════════════════════════
    ...
    with top_left:
        ...
        if st.button("Herbavital (višestranični računi)", key="goto_herbavital"):
            st.session_state.page = "herbavital"
            st.query_params["page"] = "herbavital"
            st.rerun()
...
# ═══════════════════════════════════════════
# HERBAVITAL PAGE (višestranični KIF računi)
# ═══════════════════════════════════════════
elif st.session_state.page == "herbavital":
    ...
    div.stDownloadButton > button {background:#0e8a3e; color:white; border:none; border-radius:8px; font-weight:500;}
    div.stDownloadButton > button:hover {background:#0b6e31; color:white;}
    #pdf_download_h > button {background:#18181b !important; color:white !important;}
    #pdf_download_h > button:hover {background:#3f3f46 !important; color:white !important;}
    button[kind="primary"] {background:#0e8a3e !important; color:white !important; border:none !important;}
    button[kind="primary"]:hover {background:#0b6e31 !important; color:white !important;}
    </style>
    """, unsafe_allow_html=True)
    ...
    with top_left:
        if st.button("← Nazad na KIF", key="back_kif_h"):
            st.session_state.page = "kif"
            st.query_params["page"] = "kif"
            st.rerun()

        if logo_b64:
            st.markdown(f'<div class="logo-row"><img src="data:image/png;base64,{logo_b64}" /><div class="app-title">Herbavital KIF — BS BIRO</div></div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="logo-row"><div class="app-title">Herbavital KIF — BS BIRO</div></div>', unsafe_allow_html=True)

        st.caption("Knjiga Izlaznih Faktura — višestranični računi")
        st.markdown("""
        <div class="steps">
            <strong>Kako koristiti:</strong><br>
            1. Upload-uj Herbavital PDF račune (mogu imati više stranica po računu)<br>
            2. Klikni <b>Obradi račune</b> — AI automatski spaja stranice i izvlači podatke<br>
            3. Pregledaj i edituj podatke u tabeli ako treba<br>
            4. Preuzmi gotov Excel ili CSV fajl
            <div class="copyright">Sva prava zadržana, Amir Basic - basic.amir99@gmail.com</div>
        </div>
        ...
        uploaded_files_h = st.file_uploader("Prevuci ili odaberi Herbavital PDF račune", type=["pdf"], accept_multiple_files=True, key="herbavital_uploader")

    with top_left:
        if not uploaded_files_h:
            st.info("Dodaj Herbavital račune za početak obrade.")
            st.stop()

        st.write(f"**{len(uploaded_files_h)}** fajl(ova) odabrano")

        if st.session_state.get("h_logs"):
            for t, msg in st.session_state.h_logs:
                if t == "err":
                    st.error(msg, icon="❌")
                elif t == "warn":
                    st.warning(msg, icon="⚠️")
    ...
    if "h_logs" not in st.session_state:
        st.session_state.h_logs = []
    if "h_pdf_map" not in st.session_state:
        st.session_state.h_pdf_map = {}
    if "h_labels" not in st.session_state:
    ...
    if st.session_state.h_results:
        with top_left:
            ...
            df = pd.DataFrame(st.session_state.h_results, columns=KIF_HEADERS)
            for col in KIF_HEADERS:
                if col not in df.columns:
                    df[col] = ""

            edited_df = st.data_editor(df[KIF_HEADERS], use_container_width=True, hide_index=True, num_rows="dynamic", key="data_editor_h")

            def create_xls_h(dataframe):
                wb = xlwt.Workbook(encoding="utf-8")
                ws = wb.add_sheet("Racuni")
                for c, h in enumerate(KIF_HEADERS):
                    ws.write(0, c, h)
            ...
            def create_dbf_h(dataframe):
                return _write_dbf(dataframe, KIF_HEADERS)

            st.divider()
            e1, e2, e3 = st.columns(3)
            with e1:
                st.download_button("Preuzmi DBF", create_dbf_h(edited_df), "herbavital.dbf", type="primary", use_container_width=True)
            with e2:
                st.download_button("Preuzmi XLS", create_xls_h(edited_df), "herbavital.xls", use_container_width=True)
            with e3:
                st.download_button("Preuzmi CSV", edited_df.to_csv(index=False, sep=";", encoding="utf-8-sig"), "herbavital.csv", use_container_width=True)

        with top_right:
            st.subheader("PDF pregled")

            def preview_label_h(i):
                r = st.session_state.h_results[i]
                naziv = r.get("NAZIVPP", "?")
                broj = r.get("BRDOKFAKT", "")
                src = st.session_state.h_labels.get(i, "")
                parts = [naziv]
                if broj:
                    parts.append(f"#{broj}")
                if src:
            ...
            selected = st.selectbox(
                "Odaberi račun za pregled",
                options=range(len(st.session_state.h_results)),
                format_func=preview_label_h,
                key="herbavital_preview_select",
            )
            pdf_bytes = st.session_state.h_pdf_map.get(selected)
            if pdf_bytes:
                st.download_button("Preuzmi ovaj PDF", pdf_bytes, "herbavital_racun.pdf", use_container_width=True, key="pdf_download_h")
                pages = convert_from_bytes(pdf_bytes, dpi=150)
                for page in pages:
                    st.image(page, use_container_width=True)
